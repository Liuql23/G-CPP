function [y,pChange,My_seed] = f_EmbeddingSimulator_seed(x, rhoP1, rhoM1,m,seed,randseed)
%% --------------------------------------------------------------------------------------------------------------------------
...
% This can be achieved in practice using "Multi-layered  syndrome-trellis codes" (ML STC) that are asymptotically aproaching the bound.
    n = numel(x);  %计算像素总数
    lambda = calc_lambda(rhoP1, rhoM1, m, n);%计算嵌入修改概率
    ...
    if randseed == 0
        RandStream.setGlobalStream(RandStream('mt19937ar','seed',seed));%全局随机数生成器
        My_seed = seed;
    else
        My_seed = sum(100*clock);
        RandStream.setGlobalStream(RandStream('mt19937ar','Seed',My_seed));  
    end 
